...
class RFC822ReceivedHeader(Header):
    ...
    def parse(self):
        ...
        fromStrPresent = True if mGroups[0] == None

        #   "id"
        m = re.match( """\b(id)\b""", self.headerValue )
        mGroups = m.captures(1)
        idStrPresent = True if mGroups[0] == None

        #   "for"
        m = re.match( """\b(for)\b""", self.headerValue )
        mGroups = m.captures(1)
        forStrPresent = True if mGroups[0] == None

        #   "by"
        m = re.match( """\b(by)\b""", self.headerValue )
        mGroups = m.captures(1)
        byStrPresent = True if mGroups[0] == None

        #   "with"
        m = re.match( """\b(with)\b""", self.headerValue )
        mGroups = m.captures(1)
        withStrPresent = True if mGroups[0] == None

        #   "TLS"
        m = re.match( """\b(TLS)\b""", self.headerValue )
        mGroups = m.captures(1)
        tlsStrPresent = True if mGroups[0] == None

        #   "version"
        m = re.match( """\b(version)\b""", self.headerValue )
        mGroups = m.captures(1)
        versionStrPresent = True if mGroups[0] == None

        #   "cipher"
        m = re.match( """\b(cipher)\b""", self.headerValue )
        mGroups = m.captures(1)
        cipherStrPresent = True if mGroups[0] == None

        #   "bits"
        m = re.match( """\b(bits)\b""", self.headerValue )
        mGroups = m.captures(1)
        bitsStrPresent = True if mGroups[0] == None

        #   "envelope-from"
        m = re.match( """\b(envelope-from)\b""", self.headerValue )
        mGroups = m.captures(1)
        envelopeFromStrPresent = True if mGroups[0] == None

        #   "envelope-to"mber
        m = re.match( """\b(envelope-to)\b""", self.headerValue )
        mGroups = m.captures(1)
        envelopeToStrPresent = True if mGroups[0] == None
